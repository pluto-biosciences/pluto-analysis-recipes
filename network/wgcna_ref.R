################################################################################
#####    Weighted gene co-expression network analysis (reference group)    #####
################################################################################
...
# In this version of WGCNA, traits are releveled to set a reference group
# (ref_group), meaining that the specified reference group will be the baseline
# level for all pairwise comparisons.
# i.e., compare all trait levels against a specified reference group
...
# Note: this must be a categorical group listed in the tested_trait column
ref_group <- "Control"
...
plot_methods <- paste0(
    ...
    "All pairwise combinations of traits relative to the reference of ",
    ref_group, " were binarized. Pearson correlations were calculated for ",
    "statistical significance within each comparison relative to ", ref_group,
    " and plotted using the CorLevelPlot R package. Asteriks indicate level",
    "of significance: * < 0.05, ** < 0.01, *** < 0.001, while values ",
    "indicate either a positive correlation (+) or an anti-correlation (-) ",
...
# Change reference group to be in front of level
sample_data[, tested_trait] <- relevel(
...
# Update to pull out the associations based on the reference group
binarized_assocs_vs_ref <- binarized_assocs %>% select(ends_with(ref_group))

# Add sample names to rownames; should be the same order as sample_data
rownames(binarized_assocs_vs_ref) <- rownames(sample_data)
...
eigengene_trait_assocs_pvals <- ""
initialized <- FALSE
for (comps in names(binarized_assocs_vs_ref)) {
    if (initialized == TRUE) {
        print(comps)
        tmp_df <- as.data.frame(eigengene_trait_assocs[, comps], drop = FALSE)
        names(tmp_df) <- comps
        totalSamplesInCorr <- colSums(
            is.na(binarized_assocs_vs_ref[comps]) == FALSE
        )
        print(totalSamplesInCorr)
        tmp_pval <- corPvalueStudent(as.matrix(tmp_df), totalSamplesInCorr)
        eigengene_trait_assocs_pvals <- cbind(
            eigengene_trait_assocs_pvals, tmp_pval
        )
    } else {
        print(comps)
        tmp_df <- as.data.frame(eigengene_trait_assocs[, comps], drop = F)
        names(tmp_df) <- comps
        totalSamplesInCorr <- colSums(
            is.na(binarized_assocs_vs_ref[comps]) == FALSE
        )
        ...
        eigengene_trait_assocs_pvals <- corPvalueStudent(
            as.matrix(tmp_df), totalSamplesInCorr
        )
        initialized <- TRUE
    }
}
...
vis_assoc_data_ref <- merge(
    module_eigengenes, binarized_assocs_vs_ref,
    by = "row.names"
)
rownames(vis_assoc_data_ref) <- vis_assoc_data_ref$Row.names

# Select only the columns you want to visualize
vis_assoc_data_ref <- vis_assoc_data_ref[, c(
    names(module_eigengenes), names(binarized_assocs_vs_ref)
)]

# Generate a heatmap of the p-values and correlations
cor_plot <- CorLevelPlot(vis_assoc_data_ref,
    x = names(binarized_assocs_vs_ref), # column names of the traits
...
# This calculates the p-values. Note that totalSamples will need to be updated
# for the association. When not using the "all" association, NAs should not
# count towards the sample count when phenotypes are binarized.
top_genes_to_plot <- c()
for (comp in colnames(binarized_assocs_vs_ref)) {
    print(paste("Running", comp))
    samples <- rownames(
        binarized_assocs_vs_ref[which(
            is.na(binarized_assocs_vs_ref[, comp]) == FALSE
        ), ]
    )
    counts <- t(normalized_data)[samples, ]
    gene_sig_corr <- cor(
        counts, binarized_assocs_vs_ref[samples, comp],
